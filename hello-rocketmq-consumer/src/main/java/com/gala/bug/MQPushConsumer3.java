...
@Component
public class MQPushConsumer3 implements MessageListenerConcurrently {
    private  static final Logger LOGGER = LoggerFactory.getLogger(MQPushConsumer3.class);
...
//            consumer.subscribe("TopicTest","async");
    ...
    public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs, ConsumeConcurrentlyContext context) {
        ...
        try {
            for(;index<msgs.size();index++){
                ...
                System.out.printf("MQPushConsumer3消费者监听： queueID:%d Messages:%s Tags:%s%n",  msgs.get(index).getQueueId(),messageBody,msgs.get(index).getTags());
            }
        }catch (Exception e){
